...
fn get_previous_in_sequence(sequence: &Vec<i64>) -> i64 {
    let diff_sequence: Vec<i64> = sequence
        .windows(2)
        .map(|w| w[1] - w[0])
        .collect();
    if diff_sequence.iter().all(|n| *n == 0) {
        *sequence.first().expect("Sequence is empty!")
    } else {
        sequence.first().expect("Sequence is empty!") - get_previous_in_sequence(&diff_sequence)
    }
}
...
fn main() {
    ...
    for sequence in data.iter() {
        sum_new_values += get_previous_in_sequence(sequence);
    }
...
fn test_get_previous_in_sequence() {
    let sequence_1: Vec<i64> = vec![0, 3, 6, 9, 12, 15];
    assert_eq!(get_previous_in_sequence(&sequence_1), -3);

    let sequence_2: Vec<i64> = vec![1, 3, 6, 10, 15, 21];
    assert_eq!(get_previous_in_sequence(&sequence_2), 0);

    let sequence_3: Vec<i64> = vec![10, 13, 16, 21, 30, 45];
    assert_eq!(get_previous_in_sequence(&sequence_3), 5);
}
